...
# ==================================================================
#  2b. DESPESES · lector anual, projecció i detecció
#      (l'històric de despeses d'ABSIS és ANUAL: una fila per
#       aplicació i any, amb Crèd.inicial, Crèd.definitiu, ORN i
#       Pagaments; sense detall mensual)
# ==================================================================
def llegir_historic_despeses(xls_bytes, nom_fulla):
    """
    Llegeix '1_2_1 HISTÒRIC DESPESES' (pivot anual d'ABSIS).
    Offsets dins de cada bloc d'any (des de col0):
      +1 Orgànica, +2 Programa, +3 Econòmica, +4 Descripció,
      +5 Crèdits inicials, +6 Crèdits definitius, +7 ORN, +8 Pagaments
    Retorna DataFrame [aplicacio, descripcio, any, cred_ini, cred_def, orn, pag]
    amb aplicacio = 'Orgànica.Programa.Econòmica'.
    """
    wb = openpyxl.load_workbook(io.BytesIO(xls_bytes), read_only=True, data_only=True)
    ws = wb[nom_fulla]
    ...
    for i, row in enumerate(rows[:40]):
        m = [v for v in row if isinstance(v, str) and "Exercici" in v and ":" in v
             and any(ch.isdigit() for ch in v.split(":")[-1])]
        if len(m) >= 3:
            fila_anys = i
            break
    if fila_anys is None:
        raise ValueError("No s'ha trobat la fila de blocs d'anys a l'històric de despeses.")

    blocs = []
    for j, v in enumerate(rows[fila_anys]):
        if isinstance(v, str) and "Exercici" in v and ":" in v:
            try:
                blocs.append((j, int(str(v).split(":")[-1].strip())))
            except ValueError:
    ...
    dades = []
    for col0, any_ in blocs:
        for fila in rows[fila_anys + 5:]:
            def g(off):
                i = col0 + off
                return fila[i] if 0 <= i < len(fila) else None
            org, prog, econ, desc = g(1), g(2), g(3), g(4)
            if econ is None or (isinstance(econ, str) and str(econ).startswith("Total")):
                continue
            if prog is not None and str(prog).startswith("Total"):
                continue
            ecs = str(econ).strip()
            if not (ecs.isdigit() and len(ecs) >= 3):
                continue
            apl = f"{str(org).strip()}.{str(prog).strip()}.{ecs}"
            dades.append({"aplicacio": apl,
                          "descripcio": str(desc or "").strip(),
                          "any": any_,
                          "cred_ini": num(g(5)), "cred_def": num(g(6)),
                          "orn": num(g(7)), "pag": num(g(8))})

    df = pd.DataFrame(dades)
    if df.empty:
        raise ValueError("No s'han pogut extreure dades de l'històric de despeses.")
    return df.groupby(["aplicacio", "descripcio", "any"], as_index=False).sum(numeric_only=True)


def projeccio_anual_detall(serie_anual):
    """
    Projecta el proper TOTAL ANUAL d'una sèrie anual (una dada per any).
    Prova Holt (tendència) i ARIMA(1,1,1) i tria per AIC mínim.
    Retorna (total_seguent, model, criteri).
    """
    s = pd.Series(np.asarray(serie_anual, float)).replace(0, np.nan).dropna()
    if len(s) < 5 or not STATSMODELS_OK:
        base = float(s.tail(3).mean()) if len(s) else 0.0
        return max(base, 0.0), "Mitjana-3anys", \
            "Sèrie anual curta: mitjana dels 3 darrers exercicis (sense AIC)."
    resultats, aics = {}, {}
    try:
        ...
        if np.isfinite(fc):
            resultats["Holt (tendència)"] = fc
            aics["Holt (tendència)"] = round(float(m.aic), 1)
    except Exception:
        pass
    try:
        m = SARIMAX(s, order=(1, 1, 1),
                    enforce_stationarity=False, enforce_invertibility=False).fit(disp=False)
        fc = float(m.get_forecast(1).predicted_mean.iloc[0])
        if np.isfinite(fc):
            resultats["ARIMA"] = fc
            aics["ARIMA"] = round(float(m.aic), 1)
    except Exception:
        pass
    if not resultats:
        base = float(s.tail(3).mean())
        return max(base, 0.0), "Mitjana-3anys", "Cap model convergent: mitjana 3 exercicis."
    nom = min(aics, key=aics.get)
    if len(aics) == 2:
        criteri = (f"AIC més baix: {nom} (Holt={aics.get('Holt (tendència)')} "
                   f"vs ARIMA={aics.get('ARIMA')})")
    else:
        criteri = f"Únic model convergent: {nom}"
    return max(float(resultats[nom]), 0.0), nom, criteri


def processar_despeses(hist, any_final, magnitud="orn"):
    """
    Pipeline de despeses: projecció del TOTAL ANUAL per aplicació i
    distribució mensual LINEAL (execució uniforme), perquè l'històric
    de despeses no té detall mensual. El perfil esperat és lineal (m/12).
    """
    hist = hist[hist["any"] <= any_final].copy()
    lineal = np.arange(1, 13) / 12.0
    apps = [a for a, v in hist.groupby("aplicacio")[magnitud].sum().items() if v > 0]
    resultat = {}
    for i, apl in enumerate(apps):
        g = hist[hist["aplicacio"] == apl].sort_values("any")
        total, model, criteri = projeccio_anual_detall(g[magnitud].to_numpy())
        cd = g["cred_def"].replace(0, np.nan).dropna()
        credit = float(cd.tail(3).mean()) if len(cd) else total
        resultat[apl] = {
            "desc": g["descripcio"].iloc[0],
            "proj": total * lineal,          # acumulat mensual lineal
            "perfil": lineal.copy(),          # perfil esperat lineal
            "credit": credit if credit > 0 else total,
            "model": model,
            "aic": criteri + " · Distribució mensual lineal (històric anual).",
        }
        yield i + 1, len(apps), apl, resultat


def detectar_despeses(hist, resultat, any_final):
    """
    Matriu de riscos adaptada a DESPESES amb les dades anuals reals:
      1. Despesa sobrevinguda: total projectat > 2× la mediana històrica d'ORN.
      2. Infraexecució crònica: mitjana ORN/Crèdit definitiu < 30% (3 darrers anys).
      3. Pagaments endarrerits: mitjana Pagaments/ORN < 75% (3 darrers anys).
      4. Import repetit: ORN idèntic (±1%) en dos exercicis consecutius i material.
    """
    hist = hist[hist["any"] <= any_final]
    det = {1: [], 2: [], 3: [], 4: []}
    for apl, d in resultat.items():
        g = hist[hist["aplicacio"] == apl].sort_values("any")
        orn = g["orn"].to_numpy(float)
        cd = g["cred_def"].to_numpy(float)
        pag = g["pag"].to_numpy(float)
        anys = g["any"].to_numpy()
        orn_pos = orn[orn > 0]
        total_proj = d["proj"][-1]

        # 1 · despesa sobrevinguda (alerta real en despeses)
        if len(orn_pos) >= 3:
            mediana = float(np.median(orn_pos))
            if mediana > 0 and total_proj > 2 * mediana:
                det[1].append((apl, d["desc"], str(ANY_PROJ),
                               f"projecció {total_proj:,.0f} € vs mediana {mediana:,.0f} €"))

        # 2 · infraexecució crònica (ORN / crèdit definitiu)
        mask = cd > 0
        if mask.sum() >= 2:
            ratios = orn[mask][-3:] / cd[mask][-3:]
            r_mitja = float(np.mean(ratios))
            if r_mitja < 0.30:
                det[2].append((apl, d["desc"], "3 últims",
                               f"execució mitjana {r_mitja*100:.0f}% del crèdit (llindar 30%)"))

        # 3 · pagaments endarrerits (Pagaments / ORN)
        mask2 = orn > 0
        if mask2.sum() >= 2:
            rp = pag[mask2][-3:] / orn[mask2][-3:]
            rp_mitja = float(np.mean(rp))
            if rp_mitja < 0.75:
                det[3].append((apl, d["desc"], "3 últims",
                               f"pagat {rp_mitja*100:.0f}% de l'obligat (llindar 75%)"))

        # 4 · import idèntic dos exercicis consecutius
        for i in range(len(orn) - 1):
            a, b = orn[i], orn[i + 1]
            if a > 0 and b > 0 and abs(a - b) <= 0.01 * max(a, b) and a > 1000:
                det[4].append((apl, d["desc"], f"{anys[i]}–{anys[i+1]}",
                               f"{a:,.0f} € repetit"))
                break
    return det
...
BLOCS_DET_INGRESSOS = [
    ...
    (2, "2 · Infraexecució crònica", "< 30% acumulat al mes 9 (setembre)",
     "🔴 Reavaluar la viabilitat del projecte / anul·lar crèdit.", "C00000", "FFFFFF"),
    (3, "3 · Ingressos endarrerits", "< 75% del perfil històric esperat",
     "🟡 Accions de gestió tributària o de recaptació.", "FFC000", "000000"),
    (4, "4 · Pagament/ingrés duplicat", "Dos increments mensuals consecutius idèntics",
     "🟠 Verificar duplicitat i bloquejar si escau.", "ED7D31", "FFFFFF"),
]

BLOCS_DET_DESPESES = [
    (1, "1 · Despesa sobrevinguda", "Total projectat > 2× la mediana històrica d'ORN",
     "🔴 Auditoria d'urgència de l'expedient (Interventor + Regidor).", "C00000", "FFFFFF"),
    (2, "2 · Infraexecució crònica", "Execució mitjana ORN/Crèdit def. < 30% (3 darrers exercicis)",
     "🔴 Reavaluar la viabilitat del projecte / anul·lar crèdit.", "C00000", "FFFFFF"),
    (3, "3 · Pagaments endarrerits", "Pagaments/ORN < 75% de mitjana (3 darrers exercicis)",
     "🟡 Revisar morositat i període mitjà de pagament.", "FFC000", "000000"),
    (4, "4 · Import repetit", "ORN idèntic (±1%) en dos exercicis consecutius",
     "🟠 Verificar possible duplicitat o assentament copiat.", "ED7D31", "FFFFFF"),
]
...
def construir_excel(resultat, det, any_proj=ANY_PROJ, mes_defecte=5,
                    tipus="INGRESSOS", blocs_det=None, nota_det=None):
    if blocs_det is None:
        blocs_det = BLOCS_DET_INGRESSOS
    if nota_det is None:
        nota_det = ("Nota: sèries netes de devolucions/anul·lacions d'exercicis "
                    "tancats (acumulat mai decreixent) per no distorsionar l'execució.")
    FONT = "Calibri"
    ...
    # -------------------- ANALISI --------------------
    nom_full = f"ANALISI_{tipus}_{any_proj}"
    ws = wb.create_sheet(nom_full)
    ws["A1"] = f"SEGUIMENT PRESSUPOSTARI – {tipus} (Projecció {any_proj})"
    ...
    wm.cell(row=r, column=2,
            value=f"DETECCIÓ D'ANOMALIES SOBRE LES DADES (Projecció {any_proj})")
    ...
    for num, titol, criteri, implicacio, fill_hex, tc in blocs_det:
        wm.cell(row=r, column=2, value=titol)
        wm.merge_cells(start_row=r, start_column=2, end_row=r, end_column=6)
        c = wm.cell(row=r, column=2)
        c.fill = PatternFill("solid", fgColor=fill_hex)
        c.font = Font(bold=True, color=tc, size=11, name=FONT)
        c.alignment = left
        r += 1
...
def main():
    ...
    tipus = st.radio("Tipus d'anàlisi", ["Ingressos", "Despeses"], horizontal=True,
                     help="Ingressos: projecció mensual (ETS/SARIMA per AIC) sobre "
                          "l'històric mensual. Despeses: l'històric d'ABSIS és anual, "
                          "així que es projecta el total anual (Holt/ARIMA per AIC) "
                          "amb distribució mensual lineal.")

    es_despeses = tipus == "Despeses"
    clau_fulla = ("DESPES", "") if es_despeses else ("HISTÒRIC", "INGRES")
    default_idx = 0
    for i, f in enumerate(fulles):
        fu = f.upper()
        if es_despeses:
            if "HISTÒRIC" in fu and "DESPES" in fu:
                default_idx = i
                break
        else:
            if "HISTÒRIC" in fu and "INGRES" in fu:
                default_idx = i
                break

    col_a, col_b = st.columns(2)
    with col_a:
        fulla = st.selectbox(f"Fulla d'històric de {tipus.lower()}", fulles,
                             index=default_idx,
        ...
        if es_despeses:
            magnitud = st.selectbox("Magnitud a projectar",
                                    ["ORN – Obligacions reconegudes netes",
                                     "Pagaments", "Crèdit definitiu", "Crèdit inicial"],
                                    help="ORN és l'execució de despeses (recomanat). "
                                         "Pagaments és el realment pagat.")
            mag_desp = {"ORN – Obligacions reconegudes netes": "orn",
                        "Pagaments": "pag", "Crèdit definitiu": "cred_def",
                        "Crèdit inicial": "cred_ini"}[magnitud]
        else:
            magnitud = st.selectbox("Magnitud a projectar",
    ...
    if es_despeses:
        st.caption("ℹ️ L'històric de despeses d'ABSIS és **anual** (sense detall "
                   "mensual). La projecció és del total anual i la distribució "
                   "mensual és lineal; la matriu de riscos usa els ratis reals "
                   "ORN/Crèdit i Pagaments/ORN de cada exercici.")
    ...
    if es_despeses:
        try:
            with st.spinner("Extraient l'històric de despeses…"):
                hist = llegir_historic_despeses(xls_bytes, fulla)
        except Exception as e:
            st.error(f"No s'ha pogut llegir l'històric de despeses: {e}")
            st.stop()
        ...
        resultat = {}
        for fet, total, apl, resultat in processar_despeses(hist, any_proj - 1, mag_desp):
            barra.progress(fet / total, f"Projectant aplicacions… {fet}/{total}")
        barra.empty()
        with st.spinner("Detectant anomalies i construint l'Excel…"):
            det = detectar_despeses(hist, resultat, any_proj - 1)
            xlsx = construir_excel(
                resultat, det, any_proj, mes_defecte,
                tipus="DESPESES", blocs_det=BLOCS_DET_DESPESES,
                nota_det=("Nota: l'històric de despeses és anual; la detecció usa "
                          "els ratis reals ORN/Crèdit definitiu i Pagaments/ORN "
                          "de cada exercici."))
        nom_sortida = f"ANALISI_DESPESES_{any_proj}.xlsx"
        etiq = [("Despesa sobrevinguda", 1), ("Infraexecució", 2),
                ("Pag. endarrerits", 3), ("Import repetit", 4)]
    else:
        try:
        ...
        with st.spinner("Detectant anomalies i construint l'Excel…"):
            ...
            xlsx = construir_excel(resultat, det, any_proj, mes_defecte,
                                   tipus="INGRESSOS")
        nom_sortida = f"ANALISI_INGRESSOS_{any_proj}.xlsx"
        etiq = [("Pics meritació", 1), ("Infraexecució", 2),
                ("Ingr. endarrerits", 3), ("Possibles duplicats", 4)]
    ...
    cols_m = st.columns(min(3, max(1, len(models))))
    for cm, (k, v) in zip(cols_m * 3, models.most_common()):
        cm.metric(k, v)

    st.write("Anomalies detectades sobre la projecció:")
    a1, a2, a3, a4 = st.columns(4)
    for col, (nom, num) in zip((a1, a2, a3, a4), etiq):
        col.metric(nom, len(det[num]))

    st.write("")
    st.download_button(
        "⬇  Descarregar l'Excel d'anàlisi",
        data=xlsx,
        file_name=nom_sortida,
